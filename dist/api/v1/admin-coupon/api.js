...
router.get("/coupons/list", checkCookies, asyncHandler(async (req, res) => {
...
router.get("/coupon/:couponId", checkCookies, asyncHandler(async (req, res) => {
...
router.post("/coupon/create", checkCookies, InputSensitization, asyncHandler(async (req, res) => {
    const { code, discount, minOrderValue, usageLimit, validFrom, validTill, isActive, applicableTo, } = req.body;
    const productExists = await Product.find({
        _id: {
            $in: applicableTo.map((id) => new mongoose.Types.ObjectId(id)),
        },
    });
...
router.patch("/coupon/edit/:couponId", checkCookies, InputSensitization, asyncHandler(async (req, res) => {
    const couponId = req.params.couponId;
    if (!couponId) {
        return response.failure(res, "Coupon ID is required", 400);
    }
    const updatedCoupon = await Coupon.findByIdAndUpdate(couponId, { $set: req.body }, { new: true }).lean();
    if (!updatedCoupon) {
        return response.failure(res, "Coupon not found", 404);
    }
    return response.success(res, "Coupon updated successfully", 200, updatedCoupon);
}));
router.delete("/coupon/delete/:couponId", checkCookies, asyncHandler(async (req, res) => {
    const couponId = req.params.couponId;
    if (!couponId) {
        return response.failure(res, "Coupon ID is required", 400);
    }
    ...
    return response.success(res, "Coupon deleted successfully", 200, deletedCoupon);
}));
router.get("/coupons/apply", 
//   checkCookies,
asyncHandler(async (req, res) => {
    const { code, orderValue, productId } = req.query;
    if (!code || !orderValue || !productId) {
        return response.failure(res, "Code, order value, and product ID are required", 400);
    }
    ...
    if (currentDate < new Date(coupon.validFrom) ||
