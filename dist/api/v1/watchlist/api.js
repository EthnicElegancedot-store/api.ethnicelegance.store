...
router.get("/:productId", checkCookies, asyncHandler(async (req, res) => {
    ...
    return response.success(res, "Product added to watchlist successfully", 202, newWatchlist);
}));
router.get("/:productId", checkCookies, asyncHandler(async (req, res) => {
    const productId = req.params.productId;
    if (!productId) {
    ...
    return response.success(res, "Watchlist get successfully", 202, isExistWatchlist);
}));
router.delete("/:productId", checkCookies, asyncHandler(async (req, res) => {
    const productId = req.params.productId;
    if (!productId) {
        return response.failure(res, "Product ID is required", 404);
    }
    ...
    return response.success(res, "Product removed from watchlist successfully", 202, isExistWatchlist);
}));
