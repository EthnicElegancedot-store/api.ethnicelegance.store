...
router.get("/create/:productId", checkCookies, asyncHandler(async (req, res) => {
    ...
    return response.success(res, "Product added to cart successfully", 202, addToCart);
}));
router.post("/update/:productId", checkCookies, asyncHandler(async (req, res) => {
    const productId = req.params.productId;
    if (!productId) {
        return response.failure(res, "Product ID is required", 404);
    }
    ...
    const updatedCart = await AddToCart.findOneAndUpdate({
        ...
        $set: {
            ...
            "items.$.totalPrice": Math.max(1, req.body.quantity) * productExist.price,
        },
    }, { new: true });
    return response.success(res, "Product quantity updated successfully", 202, updatedCart);
}));
router.get("/all", checkCookies, asyncHandler(async (req, res) => {
    ...
    return response.success(res, "Add to cart get successfully", 200, cart.items);
}));
router.delete("/delete/:productId", checkCookies, asyncHandler(async (req, res) => {
    const productId = req.params.productId;
    if (!productId) {
        return response.failure(res, "Product ID is required", 404);
    }
    ...
    return response.success(res, "Product removed from cart successfully", 202, isExistCart);
}));
