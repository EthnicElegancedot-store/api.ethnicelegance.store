...
// Sign Up
router.post("/sign-up", checkCookies, InputSensitization, asyncHandler(async (req, res) => {
    ...
    const { otp, otpSession } = await generateOTP("auth", {
        name,
        email,
        password,
...
router.post("/verify-email", checkCookies, InputSensitization, asyncHandler(async (req, res) => {
    ...
    const token = createAuthToken(newUser._id.toString(), newUser.email, newUser.role);
...
// Login
router.post("/login", checkCookies, InputSensitization, asyncHandler(async (req, res) => {
...
// Logout
router.get("/logout", checkCookies, asyncHandler(async (req, res) => {
    res.clearCookie("token");
    return response.success(res, "Logged out successfully", 200);
}));
// Request OTP for forgot password
router.post("/forgot-password", checkCookies, InputSensitization, asyncHandler(async (req, res) => {
...
router.post("/verify-forgot-password", checkCookies, InputSensitization, asyncHandler(async (req, res) => {
    const { email, code } = req.body;
    const isEmailCached = await checkOTP("pass", email, code);
    if (!isEmailCached.status) {
        return response.failure(res, isEmailCached.message, 400);
    }
    const hashedPassword = await User.hashPassword(isEmailCached.data.newPassword);
    ...
    const token = createAuthToken(updatedUser._id.toString(), updatedUser.email, updatedUser.role);
    res.cookie("token", token, {
        httpOnly: true,
        // secure: process.env.NODE_ENV === "production",
        // sameSite: "strict",
    ...
    return response.success(res, "Password updated successfully, Logged in automatically", 200);
}));
