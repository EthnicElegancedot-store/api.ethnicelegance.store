...
router.get("/me", checkCookies, asyncHandler(async (req, res) => {
    const user = await User.findById(req?._id).select("-password -__v -createdAt -updatedAt");
    return response.success(res, "User fetched successfully", 200, user);
}));
// edit user account
router.patch("/edit", checkCookies, asyncHandler(async (req, res) => {
    if (req.body.password) {
        return response.failure(res, "Password cannot be changed here", 400, null);
    }
    if (req.body.email) {
    ...
    }, { new: true }).select("-password -__v -createdAt -updatedAt -role");
    return response.success(res, "User updated successfully", 200, updateUser);
}));
// create address for user
router.post("/address/create", checkCookies, InputSensitization, asyncHandler(async (req, res) => {
    ...
    return response.success(res, "Address created successfully", 201, newAddress);
}));
// edit address for user
router.patch("/address/edit/:addressId", checkCookies, asyncHandler(async (req, res) => {
    ...
    return response.success(res, "Address updated successfully", 200, updatedAddress);
}));
// delete address for user
router.delete("/address/delete/:addressId", checkCookies, asyncHandler(async (req, res) => {
    const addressId = req.params.addressId;
    if (!addressId) {
        return response.failure(res, "Address ID is required", 400, null);
    }
    ...
    return response.success(res, "Address deleted successfully", 200, deletedAddress);
}));
